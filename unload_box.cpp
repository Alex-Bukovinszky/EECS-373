...
int main(int argc, char** argv) {
    ...
    ROS_INFO("instantiating a BinInventory");  //added for BinInventory
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
       ...
       //this converts to the world coordinates and retrieves names of objects, updates bad parts name
	//also converts
        for (int i=0;i<nparts;i++) {
        ...
        //gets name by iterating through parts
        cout<<"enter 1 to remove bad part then other parts"; //poor-man's breakpoint
        cin>>ans;        
    ...
    nparts = orphan_models_wrt_world.size();
    ROS_INFO("num parts seen in box = %d",nparts);
    cout<<"enter 1 to remove other parts: "; //poor-man's breakpoint
    cin>>ans;
    // Print out then remove each part
    for (int i=0;i<nparts;i++) {
       ROS_INFO_STREAM("orphaned  parts: "<<orphan_models_wrt_world[i]<<endl);
